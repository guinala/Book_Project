...
export default function BookGridCard({ book, rank }: BookGridCardProps) {
  ...
  return (
    <article
      className={`book-grid-card${dropdownOpen ? " book-grid-card--open" : ""}`}
      onClick={handleCardClick}
    >
      <div className="book-grid-card__cover-wrapper">
        {hasCover ? (
          <img
            className="book-grid-card__cover"
            src={coverSrc}
          ...
          <div className="book-grid-card__cover-placeholder">
        ...
        {rank && <span className="book-grid-card__rank">{rank}</span>}
      </div>
      ...
      <div className="book-grid-card__save-wrapper" ref={wrapperRef}>
        ...
        >
          {saved && !dropdownOpen ? (
            <svg className="book-grid-card__save-icon" viewBox="0 0 24 24" fill="currentColor" stroke="none">
              <path d="M5 3a2 2 0 0 0-2 2v16l9-6 9 6V5a2 2 0 0 0-2-2H5z"/>
            </svg>
          ) : (
            <svg className={`book-grid-card__save-icon${dropdownOpen ? " book-grid-card__save-icon--open" : ""}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round">
        ...
        {dropdownOpen && (
          <ul className="book-grid-card__dropdown" onClick={(e) => e.stopPropagation()}>
            {SHELF_OPTIONS.map((opt) => (
              <li key={opt}>
                <button
                  className={`book-grid-card__dropdown-item${saved === opt ? " book-grid-card__dropdown-item--active" : ""}`}
      ...
      <div className="book-grid-card__info">
        <div className="book-grid-card__text">
          <h3 className="book-grid-card__title">{book.title}</h3>
          <p className="book-grid-card__author">
